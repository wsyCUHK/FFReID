...
class PatchEmbed_overlap(nn.Module):
    ...
    """
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, embed_dim=768, stem_conv=False):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
...
class TransReID(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, local_feature=False, sie_xishu =1.0, stem_conv=True):
        super().__init__()
    ...
    def forward_features(self, x, camera_id, view_id):
        B = x.shape[0]
        x = self.patch_embed(x)  # 将二维image转化成一维
        ...
        if self.cam_num > 0 and self.view_num > 0:
            ...
            x = x + self.pos_embed      # position Embedding, self.pos_embed是可学习的 position embedding
    ...
    def load_param(self, model_path):
        ...
        # if 'base' in param_dict:
        #     param_dict = param_dict['base']
        # for k in list(param_dict.keys()):
        #     if 'base' in k:
        #         tmp_k = k[5:]
        #         # print('tmp_k', tmp_k)
        #         param_dict[tmp_k] = param_dict.pop(k)
                
        #         # print('para_dict', param_dict['base'])
...
# 改变输入图像大小 vit不会改变patches大小，patches数量会改变，下面是插值的方法
...
# def vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_rate=0.0, attn_drop_rate=0.0, drop_path_rate=0.1, camera=0, view=0,local_feature=False,sie_xishu=1.5, **kwargs):
#     model = TransReID(
#         img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=768, depth=11, num_heads=12, mlp_ratio=4, qkv_bias=True,\
#         camera=camera, view=view, drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate,
#         norm_layer=partial(nn.LayerNorm, eps=1e-6),  sie_xishu=sie_xishu, local_feature=local_feature, **kwargs)
